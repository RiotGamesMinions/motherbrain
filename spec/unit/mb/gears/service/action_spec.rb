...
describe MB::Gear::Service::Action do
  ...
  let(:node_1) { double('node_1', name: 'reset', public_hostname: 'reset.riotgames.com', public_ipv4: '0.0.0.0') }
  let(:node_2) { double('node_2', name: 'jwinsor', public_hostname: 'jwinsor.riotgames.com', public_ipv4: '0.0.0.0') }
  let(:node_3) { double('node_3', name: 'jwinsor-2', public_hostname: 'jwinsor-2.riotgames.com', public_ipv4: '0.0.0.0') }
  ...
  describe "#run" do
    ...
    it "runs Chef on every node" do
      MB::Application.node_querier.should_receive(:chef_run).with(node_1.public_hostname, node_1.public_ipv4).
        and_return(chef_success)
      MB::Application.node_querier.should_receive(:chef_run).with(node_2.public_hostname, node_2.public_ipv4).
        and_return(chef_success)
      MB::Application.node_querier.should_receive(:chef_run).with(node_3.public_hostname, node_3.public_ipv4).
        and_return(chef_success)
